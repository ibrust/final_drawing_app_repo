...
//  drawing_app_revised
//
//  Created by Field Employee on 11/27/20.
//
    ...
    func fetch(_ url: URL) {
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            if let data = try? Data(contentsOf: url.imageURL) {
                if self != nil {  // check if ok to create image on main thread
...
extension URL {
    var imageURL: URL {
        if let url = UIImage.urlToStoreLocallyAsJPEG(named: self.path) {
            // this was created using UIImage.storeLocallyAsJPEG directory
            return url
        } else {
